...
public class Boss : MonoBehaviour
{
    ...
    public Boss_Missile prefab; // 투사체
    ...
    IEnumerator CircularImpulse()
    {
        ...
        animator.SetBool("isCurcular", true);

        yield return new WaitForSeconds(1f);
        animator.SetBool("isCurcular", false);
    ...
    {
        float dx = (Mathf.Abs(targetPos.x) - Mathf.Abs(transform.position.x));
        float dy = (Mathf.Abs(targetPos.y) - Mathf.Abs(transform.position.y));
        float rotateDg = Mathf.Atan2(dx, dy) * Mathf.Rad2Deg + 90;
